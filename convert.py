...
# Copyright 2024, Ingo Breßler (ingo.bressler@bam.de)
#
...
def genIOCconfig(motors: List[dict], imsPath: Path, outpath: Path):
    ...
    for addressWithPort, motors in motorsByAddress.items():
        port = int(addressWithPort.split(":")[-1])
...
def main(args: List[str] = None):
    ...
    print(args)
    for key in ("configAdm", "addressesAliases", "limits", "imsPath"):
        if not vars(args).get(key):
            raise ValueError(f"Please provide a '{key}' file, see '-h' option for details.")
    ...
    genIOCconfig(motors, args.imsPath, outpath)
    print("done.")
